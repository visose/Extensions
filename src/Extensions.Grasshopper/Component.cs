...
public abstract class Component(
    ...
    : GH_Component(name, nickname, description, "Extensions", subcategory)
{
    protected Component(
        string name,
        string description,
        ...
        string icon,
        GH_Exposure exposure = GH_Exposure.primary)
        : this(name, InferNickname(name), description, subcategory, id, icon, exposure) { }
    ...
    {
        var message = exception is XmlException xml
...
public abstract class RobotComponent(
    ...
    string icon,
    GH_Exposure exposure = GH_Exposure.primary)
    : Component(name, nickname, description, subcategory, id, icon, exposure)
{
    public override GH_Exposure Exposure => ExtensionsInfo.IsRobotsInstalled ? base.Exposure : GH_Exposure.hidden;

    protected sealed override void RegisterInputParams(GH_InputParamManager pManager)
    {
        if (ExtensionsInfo.IsRobotsInstalled)
            RegisterRobotInputParams(pManager);
    }

    protected sealed override void RegisterOutputParams(GH_OutputParamManager pManager)
    {
        if (ExtensionsInfo.IsRobotsInstalled)
            RegisterRobotOutputParams(pManager);
    }

    protected virtual void RegisterRobotInputParams(GH_InputParamManager pManager) { }
    protected virtual void RegisterRobotOutputParams(GH_OutputParamManager pManager) { }
}
