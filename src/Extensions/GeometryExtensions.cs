...
public static class GeometryExtensions
{
    extension(Curve curve)
    {
        public Polyline ToPolyline()
        {
            if (!curve.TryGetPolyline(out Polyline polyline))
                throw new ArgumentException("Curve is not a polyline.", nameof(curve));

            return polyline;
        }
    }

    extension(IList<Point3d> pl)
    {
    ...
    extension(Polyline pl)
    {
        public Point3d ClosestPointFast(Point3d testPoint)
        {
            int count = pl.Count;
